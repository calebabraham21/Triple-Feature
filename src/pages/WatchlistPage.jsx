...
const WatchlistPage = () => {
  const { watchlist, removeFromWatchlist, loading, error } = useWatchlist();
  const [filter, setFilter] = useState('all'); // 'all', 'watched', 'unwatched'
  const [showClearConfirm, setShowClearConfirm] = useState(false);

  // Transform watchlist data to match MovieCard expectations
  const transformedMovies = watchlist.map(movie => ({
    id: movie.movie_id,
    title: movie.title,
    ...
    // Add watched status (default to false since we don't have this in DB yet)
    watched: false,
  ...
  const watchedCount = transformedMovies.filter(movie => movie.watched).length;
  const unwatchedCount = transformedMovies.filter(movie => !movie.watched).length;
  ...
  const handleToggleWatched = (movieId) => {
    // This functionality would need to be added to the database
    // For now, we'll just show a message
    console.log('Toggle watched functionality not yet implemented');
  };
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          {loading ? (
            ...
            <p className="text-red-300">Error loading watchlist: {error}</p>
          ) : (
            <>
              ...
              {watchlist.length === 0 && (
                <p className="text-white/70 text-sm mt-2">
                  Movies you add from recommendations will appear here
                </p>
              )}
            </>
          )}
        ...
        {loading && (
          <div className="text-center py-12">
            ...
            <p className="text-white">Please wait while we fetch your saved movies.</p>
          </div>
        )}
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-8">
              ...
              {watchlist.length > 0 && (
                <motion.button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-red-500/20 hover:bg-red-500/30 text-red-300 border border-red-500/30 rounded-lg transition-colors duration-200"
                >
                  <Trash2 size={16} />
                  Clear Watchlist
                </motion.button>
            ...
            {filteredMovies.length > 0 ? (
              ...
              >
                ...
                <h3 className="text-xl font-semibold mb-2">
                  {watchlist.length === 0 ? 'Your watchlist is empty' : 'No movies match your filter'}
                </h3>
                <p className="text-white">
                  {watchlist.length === 0 
                    ? 'Start by getting movie recommendations on the home page, then add your favorites to your watchlist!'
                ...
                {watchlist.length === 0 && (
                  <motion.button
                    ...
                    className="mt-4 px-6 py-2 bg-accent-blue hover:bg-accent-blue/80 text-white rounded-lg transition-colors"
                  >
                    Get Recommendations
                  </motion.button>
                )}
